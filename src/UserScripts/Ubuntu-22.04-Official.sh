...
if lsmod | grep -q "dxgkrnl" || dkms status | grep -q "dxgkrnl"; then
    ...
    echo " -> Downloading Kernel Source ZIP using Aria2 (Multi-threaded)..."
    ZIP_URL="https://github.com/microsoft/WSL2-Linux-Kernel/archive/refs/heads/$TARGET_BRANCH.zip"
    
    download_kernel() {
        # -x 4: 4线程, -s 4: 分4块, -k 1M: 块大小, --file-allocation=none: 防止预分配慢
    ...
    if [ "$SUCCESS" = false ]; then
        echo " -> [ERROR] Failed to download valid kernel source after multiple attempts."
        exit 1
    fi
