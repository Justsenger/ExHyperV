    ...
    public class DDAService : IHyperVService
    {
        ...
        public async Task<(List<DeviceInfo> Devices, List<string> VmNames)> GetDdaInfoAsync()
        {
            ...
            try
            {
                ...
                var hypervModule = await Utils.Runs("Get-Module -ListAvailable -Name Hyper-V");
                if (hypervModule != null && hypervModule.Count != 0)
                {
                    var vms = await Utils.Runs(@"Get-VM | Select-Object Name");
                    if (vms != null)
                    {
                        foreach (var vm in vms)
                        {
                            ...
                            vmNameList.Add(name);
                            var assignedDevices = await Utils.Runs($@"Get-VMAssignableDevice -VMName '{name}' | Select-Object InstanceID");
                ...
                var pnpDevices = await Utils.Runs("Get-PnpDevice | Where-Object { $_.InstanceId -like 'PCIP\\*' } | Select-Object InstanceId, Status");
                if (pnpDevices != null)
                {
                ...
                var pciData = await Utils.Runs(getPciDevicesScript);
                if (pciData != null)
                {
        ...
        {
            var result = await Utils.Runs("(Get-CimInstance -Class Win32_OperatingSystem).ProductType");
